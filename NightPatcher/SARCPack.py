...
class SimpleArchive(object):
    def pack(self, folder, padding):
        ...
        sarc.write(b"SARC\x00\x14\xFE\xFF")     #Magic + Hdr Size + Byte Order
        sarc.write(struct.pack(">I", filesize ))
        sarc.write(struct.pack(">I", datastart))
        sarc.write(b"\x01\x00\x00\x00SFAT\x00\x0C") #Attribute Table
        sarc.write(struct.pack(">H", numfiles))
        sarc.write(b"\x00\x00\x00\x65") #Hash Multiplier
        strpos = 0;filepos = 0
        for x in range(numfiles): #Write the actual table
            sarc.write(struct.pack(">I", hashes[x][0]))
            sarc.write(b"\x01") #Unknown, see mk8.tockdom.com
            sarc.write(codecs.decode(hexstr(int(strpos / 4), 4), "hex").rjust(3, b"\x00")) #Ugly
        ...
        sarc.write(b"SFNT\x00\x08\x00\x00") #Name Table Header
        for x in range(numfiles): #Write all strings
            sarc.write((filedata[hashes[x][1]][0]).encode()) #Actual string
            sarc.write(b"\x00" * (filedata[hashes[x][1]][3] - len(filedata[hashes[x][1]][0])))
        sarc.write(b"\x00" * padSFAT) #Write padding
        ...
        for x in range(numfiles):
            ...
            if x != numfiles - 1: #Don't pad last file
                sarc.write(b"\x00" * (filedata[hashes[x][1]][2] - filesize))
        sarc.close()
